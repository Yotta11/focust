...
export default function Process() {
  return (
    <section id="processus" className="bg-[linear-gradient(135deg,#00C6FF_40%,#2277FF_100%)] py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="max-w-2xl">
          <span className="text-xs font-semibold uppercase tracking-[0.2em] lg:text-sm text-[var(--color-hover)]">
            Notre méthode
          </span>
        ...
        <ol className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((s, i) => (
            <li key={s.n} className="relative pl-2">
              <span className="font-display text-5xl font-semibold text-[var(--color-roi)]">{s.n}</span>
              <h3 className="mt-3 font-display text-lg font-semibold text-white">{s.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-gray-200">{s.desc}</p>
              {i < STEPS.length - 1 && (
